...
bool os_start(uint32_t systick_ticks)
{
    ...
    NVIC_SetPriority(SysTick_IRQn, 0); /* highest */
    ...
    /* Set PSP to prepared initial SP for first task */
    __set_PSP(os_curr_task->sp);

    /* Switch to PSP (Thread mode uses PSP). Keep privileged for now (0x02). */
    __set_CONTROL(0x02);
    __ISB();

    /* Start SysTick after PSP/CONTROL is set */
    uint32_t ret_val = SysTick_Config(systick_ticks);
    if (ret_val != 0) return false;

    /* Perform exception-return to start the first task (pop frame and jump to PC) */
    __asm volatile (
        "movs r0, #0\n"
        "ldr r0, =0xFFFFFFFD\n"
        "mov lr, r0\n"
...
//	static uint32_t tick_count = 0;
//	if(++tick_count >= 1000) {  // 1 second blink
//	    HAL_GPIO_TogglePin(GPIOB, GPIO_PIN_14);
//	    tick_count = 0;
//	}
	// important note when i use the toggle led code with the down implementation the led toggle doesnot work
